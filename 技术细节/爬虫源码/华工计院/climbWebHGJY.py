...
webs = []
with open('website_HGJY.csv', 'r') as file:
...
for web in webs:
    ...
    articles = soup.find("div", attrs={"class": "article-text"})
    ...
    if (title == None):
            titles = soup.find("div", attrs={"class":"article-con"})
            titles = titles.find("div", attrs={"class":"tit"})
            title = titles.text
    items = re.split(r"演讲：|Topic:|Title:|主题：|题目：|主讲人：|主 讲 人：", str(title))
    if len(items) > 1:
        title = items[1]
    ...
    items = re.split(r"报告人：|主 讲 人：|主讲人：|Speaker:|时间：|时   间：", str(speaker))
    if len(items) > 1:
        speaker = items[1]
    ...
    items = re.split(r"时间：|讲座方式与地点：", str(time))
    if len(items) > 1:
        time = items[1]
    ...
    items = re.split(r"Abstract:|摘要：|讲座简介：|报告简介：|讲座概要：|内容简介：|主讲人简介：", str(content))
    if len(items) > 1:
        content = items[1]
    ...
    items = re.split(r"专家简介：| Biography:|教授简介：|报告人简介：|主讲人介绍：|主讲人简介：", str(speaker_file))
    if len(items) > 1:
    ...
    publish_time = seize_from_key(article, publish_time_keys)
