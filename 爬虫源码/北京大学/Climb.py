...
def prase_info(url, id):
    ...
    title_key = ["演讲：(\n)*.*", "Topic.(\n)*.*", "主题.(\n)*.*"]
    tmp = seize_from_key(article_text, title_key, id)
    title = soup.find("div", attrs={"class":"article-con"})
    title = title.find("div", attrs={"class":"tit"})
    title_text = title.text
    if (tmp == None):
        tmp = str(title_text)
    if ('知存讲座' in title_text):
        is_graph = 1
    elif ('学术沙龙' in title_text):
        is_graph = 2
    elif ('前沿学术论坛' in title_text):
        is_graph = 3
    ret.append(tmp)
    ...
    #通知发布时间
    publish_time = soup.find("div",attrs = {'class' : 'article-con'}).find("div",attrs = {'class' : 'info'}).find("span").text
    ...
    #print(ret)
    if (is_graph != 0):
        ret = seize_from_graph(id, is_graph, ret)
    if (ret[2] == None):
        ret[2] = ret[6]
    if (ret[3] == None):
        print(url)
